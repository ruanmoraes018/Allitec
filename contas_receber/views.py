...
def lista_contas_receber(request):
    ...
    contas_receber = ContaReceber.objects.filter(vinc_emp=request.user.empresa)
    if not dt_ini and not dt_fim and not por_dt:
        ...
        contas_receber = contas_receber.filter(
            situacao='Aberta',
            data_vencimento__range=(primeiro_dia, ultimo_dia)
        )
        ...
        if form.is_valid():
            try:
                ...
                if request.user.is_authenticated:
                    try:
                        cr.vinc_emp = request.user.empresa  # Busca a filial do usuário logado
...
def att_conta_receber(request, id):
    cr = get_object_or_404(ContaReceber, pk=id)
...
def pagar_conta_receber(request, id):
    ...
    if cr.situacao == 'Paga':
        ...
        return redirect('/contas_receber/lista/')
    elif cr.situacao == 'Aberta':
        cr.situacao = "Paga"
        cr.dt_pag = datetime.now()
        cr.save()
        messages.success(request, 'Pagamento da Conta à Receber realizada com sucesso!')
        return redirect('/contas_receber/lista/')

...
class Contrato(models.Model):
    ...
    SITUACAO_CHOICES = [('Ativo', 'Ativo'), ('Baixada', 'Suspenso'), ('Cancelado', 'Cancelado'), ('Expirado', 'Expirado')]
    situacao = models.CharField(max_length=10, choices=SITUACAO_CHOICES, default='Ativo')
    STATUS_CHOICES = [('Pendente', 'Pendente'), ('Aprovado', 'Aprovado')]
    ...
    def esta_expirado(self):
        if not self.dt_exp: return False
        return timezone.localdate() > self.dt_exp
    def __str__(self):
        return f'Contrato {self.id} - {self.empresa.fantasia}'
    def save(self, *args, **kwargs):
        if self.dt_exp:
            if timezone.localdate() > self.dt_exp: self.situacao = 'Expirado'
            else:
                if self.situacao == 'Expirado': self.situacao = 'Ativo'

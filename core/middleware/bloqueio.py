...
class BloqueioInadimplenciaMiddleware:
    ...
    def __call__(self, request):
        if request.path.startswith('/pedidos/webhook/') or request.path.startswith('/mensalidades/webhook/'): return self.get_response(request)
        request.bloqueado = False
        request.motivo_bloqueio = ""
        request.aviso_inadimplencia = False
        request.msg_aviso = ""
        if not request.user.is_authenticated: return self.get_response(request)
        if request.user.is_superuser: return self.get_response(request)
        filial = getattr(request.user, 'filial_user', None)
        if not filial: return self.get_response(request)
        ...
        # CONTRATO
        contrato_ativo = Contrato.objects.filter(empresa=empresa, situacao='Ativo', status='Aprovado').exists()
        ...
        request.aviso_inadimplencia = False
        request.msg_aviso = ""
        mensalidades = Mensalidade.objects.filter(empresa=empresa, situacao='Aberta', dt_venc__lt=hoje)

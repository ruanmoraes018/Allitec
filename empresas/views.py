...
def lista_empresas(request):
    ...
    if tp == 'desc' and s:
        ...
        try: empresas = empresas.filter(id__icontains=s).order_by('fantasia')
        except ValueError: empresas = Empresa.objects.none()
    if dia_venc in ['05', '10', '15', '20', '25', '30']: empresas = empresas.filter(dia_venc=dia_venc)
    ...
    if f_s in ['Ativa', 'Inativa']: empresas = empresas.filter(situacao=f_s)
    if t_pes in ['Física', 'Jurídica']: empresas = empresas.filter(pessoa=t_pes)
    if reg == 'todos': num_pagina = empresas.count() or 1
    else:
    ...
    return render(request, 'empresas/lista.html', {'empresas': empresas, 's': s, 'tp': tp, 't_pes': t_pes, 'dt_ini': dt_ini, 'dt_fim': dt_fim, 'p_dt': p_dt, 'dia_venc': dia_venc, 'reg': reg})
...
def add_empresa(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                EmpresaService.criar_empresa_com_estrutura(form=form, request_files=request.FILES)
                messages.success(request, 'Empresa cadastrada com sucesso.')
                return redirect('/empresas/lista/')
            except Exception as e: messages.error(request, f'Erro ao cadastrar empresa: {str(e)}')
        error_messages = [
    ...
    return render(request, 'empresas/add.html', {'form': EmpresaForm()})
...
def att_empresa(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            else: return redirect('/empresas/lista/')
        else:
            error_messages = []
            for field in form:

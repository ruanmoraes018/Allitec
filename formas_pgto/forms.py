...
class FormaPgtoForm(forms.ModelForm):
    ...
    descricao = forms.CharField(label='Descrição', widget=forms.TextInput(attrs={'class': c_f}))
    ...
    class Meta:
        model = FormaPgto
        fields = ('situacao', 'troco', 'tipo', 'descricao', 'gera_parcelas', 'gateway', 'credenciais', 'forma_padrao',)
    ...
    def clean(self):
        ...
        if gateway and gateway != "nenhum":
            ...
            if faltando: raise forms.ValidationError(f"Campos obrigatórios: {', '.join(faltando)}")
        return cleaned
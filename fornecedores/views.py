...
def lista_fornecedores(request):
    ...
    if tp == 'desc' and s:
        ...
        try: fornecedores = fornecedores.filter(codigo__iexact=s).order_by('fantasia')
        except ValueError: fornecedores = Fornecedor.objects.none()
    if p_dt == 'Sim' and dt_ini and dt_fim:
        ...
        except ValueError: fornecedores = fornecedores.objects.none()
    if f_s in ['Ativo', 'Inativo']: fornecedores = fornecedores.filter(situacao=f_s).order_by('fantasia')
    if t_pes in ['Física', 'Jurídica']: fornecedores = fornecedores.filter(pessoa=t_pes).order_by('fantasia')
    if reg == 'todos': num_pagina = fornecedores.count() or 1  # Mostra todas as filials
    else:
    ...
    return render(request, 'fornecedores/lista.html', {'fornecedores': fornecedores, 's': s, 'tp': tp, 't_pes': t_pes, 'dt_ini': dt_ini, 'dt_fim': dt_fim, 'p_dt': p_dt, 'reg': reg})
...
def lista_fornecedores_ajax(request):
    ...
    try:
        ...
        results = [{
            'id': fornecedor.codigo, 'text': f"{fornecedor.fantasia.upper()}", 'situacao': fornecedor.situacao, 'cpfCnpj': fornecedor.cpf_cnpj, 'email': fornecedor.email,
            'tel': fornecedor.tel, 'endereco': fornecedor.endereco, 'cep': fornecedor.cep, 'numero': fornecedor.numero, 'bairro': fornecedor.bairro.nome_bairro if fornecedor.bairro else '',
            'cidade': fornecedor.cidade.nome_cidade if fornecedor.cidade else '', 'uf': fornecedor.uf.nome_estado if fornecedor.uf else ''} for fornecedor in fornecedores]
...
def att_fornecedor(request, codigo):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            else: return redirect('/fornecedores/lista/?tp=cod&s=' + clie)
        else:
            error_messages = []
            for field in form:
                if field.errors: error_messages.append(f"<i class='fa-solid fa-xmark'></i> Campo ({field.label}) é obrigatório!")
...
@login_required
def del_fornecedor(request, codigo):

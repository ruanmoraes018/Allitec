...
def lista_lancamentos(request):
    ...
    if tp == 'desc' and s:
        ...
        try:
            caixas = caixas.filter(id__iexact=s).order_by('terminal__nome')
        except ValueError:
            caixas = Caixa.objects.none()
    if sit and sit != 'Todos': caixas = caixas.filter(situacao=sit)
    if fil: caixas = caixas.filter(vinc_fil_id=fil)
    if user1: caixas = caixas.filter(usuario_id=user1)
    filiais = Filial.objects.filter(vinc_emp=request.user.empresa)
    usuarios = Usuario.objects.filter(empresa=request.user.empresa)
    if reg == 'todos':
        num_pagina = caixas.count() or 1
    else:
        try:
    ...
    return render(request, 'lancpdvs/lista.html', {
        'caixas': caixas, 'filiais': filiais, 'usuarios': usuarios, 'user1': user1, 'sit': sit, 's': s, 'tp': tp, 'reg': reg,
    })
...
def lista_lancamentos_ajax(request):
    ...
    try:
        ...
        results = [{'id': p.id, 'text': f"{p.terminal.nome.upper()}"} for p in caixas]
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                for forma in formas:
                    valor = form.cleaned_data.get(f'forma_{forma.id}') or Decimal('0')
                    if valor > 0:
                        CaixaMovimento.objects.create(caixa=caixa, tipo='Entrada', categoria='Saldo Inicial', forma_pagamento=forma, valor=valor, usuario=request.user)
            messages.success(request, 'Caixa aberto com sucesso!')
            return redirect(f'/lancpdvs/lista/?tp=cod&s={caixa.id}')
        else:
            error_messages = []
            for field in form:
                if field.errors:
        ...
        form = CaixaForm(empresa=request.user.empresa)
    formas = FormaPgto.objects.filter(vinc_emp=request.user.empresa, tipo='A vista', situacao='Ativo').values('id', 'descricao')
    return render(request, 'lancpdvs/add.html', {'form': form, 'formas_json': list(formas)})

@login_required
def att_lancamento(request, id):
    b = get_object_or_404(Caixa, pk=id, vinc_emp=request.user.empresa)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if next_url:
                ...
                return redirect('/lancpdvs/lista/?tp=cod&s=' + bank)
        else:
            error_messages = []
            for field in form:
                if field.errors:
        ...
        formas = FormaPgto.objects.filter(vinc_emp=request.user.empresa, situacao='Ativo').values('id', 'descricao')
        return render(request, 'lancpdvs/att.html', {'form': form, 'b': b, 'formas_json': list(formas)})

@login_required
def del_lancamento(request, id):
...
def tela_caixa(request, caixa_id):
    empresa = request.user.empresa
    caixa = get_object_or_404(Caixa, id=caixa_id, vinc_emp=empresa, situacao='Aberto')
...
def movimentos_caixa(request, caixa_id):
    try:
        caixa = Caixa.objects.get(id=caixa_id, vinc_emp=request.user.empresa)
        ...
        for m in vendas_qs:
            ...
            if pedido_id not in vendas_dict:
                vendas_dict[pedido_id] = {
                    "pedido_id": pedido_id, "cliente": f"{m.pedido.cli_id or '-'} - {getattr(m.pedido.cli, 'fantasia', '-')}",
                    "vendedor": f"{m.pedido.vendedor_id or '-'} - {getattr(m.pedido.vendedor, 'fantasia', '-')}", "data": m.pedido.dt_emi, "formas": [], "total": 0
                }
        ...
        for m in vendas_qs:
            fp_id = m.forma_pagamento.id if m.forma_pagamento else 0
...
def finalizar_venda(request):
    try:
        ...
        for pg in pagamentos_front:
            forma = FormaPgto.objects.get(id=pg['forma_id'])
            # 🔥 CLIENTE SOMENTE À VISTA
            if (
                cliente
                and cliente.somente_avista
                and (forma.tipo or '').strip().upper() == 'A PRAZO'
            ):
                return JsonResponse({
                    'erro': f'Cliente {cliente.fantasia} permite apenas vendas à vista.'
                }, status=400)
        # 🔥 verifica PIX pendente
        content_type = ContentType.objects.get_for_model(caixa)
        pix_pendente = Pagamento.objects.filter(
        ...
        for item in itens:
            try:
                prod = Produto.objects.get(
                    id=item['produto_id']
                )
                qtd = Decimal(str(item['qtd']))
                if (
                    hasattr(prod, 'estoque_prod')
                    and prod.estoque_prod is not None
                ):  
                    if not pode_vender_sem_estoque and prod.estoque_prod < qtd:
                        return JsonResponse({
                            'erro': f'Estoque insuficiente para o produto {prod.nome}. Disponível: {prod.estoque_prod}!'
                        }, status=400)
...
def gerar_pagamento_caixa(request):
    try:
        ...
        if not pagamentos_front:
            return JsonResponse({"erro": "Nenhuma forma enviada"}, status=400)
        # 🔥 remove PIX pendente antigo (igual pedido)
        from django.contrib.contenttypes.models import ContentType
        Pagamento.objects.filter(content_type=ContentType.objects.get_for_model(caixa), object_id=caixa.id, status="pendente").delete()
        pagamentos_gerados = []
        for pg in pagamentos_front:
            forma = FormaPgto.objects.get(id=pg["forma"])
            valor = Decimal(str(pg["valor"]))
            gateway = (forma.gateway or "").strip().lower()
            if gateway in ["", "nenhum", "none"]:
                continue
            try:
                ...
                result = service.gerar_pagamento(valor=valor, descricao=f"Venda Caixa {caixa.id}", email=None, external_reference=str(caixa.id))
                if not result:
                    continue
                txid = result.get("id")
                ...
                pagamento = Pagamento.objects.create(
                    vinc_emp=caixa.vinc_emp, content_type=ContentType.objects.get_for_model(caixa), object_id=caixa.id, forma_pgto=forma, valor=valor, txid=txid,
...
def status_pagamento_caixa(request, caixa_id):
    try:
        caixa = Caixa.objects.get(id=caixa_id, vinc_emp=request.user.empresa)
        pagamentos = Pagamento.objects.filter(content_type=ContentType.objects.get_for_model(caixa), object_id=caixa.id)
        if not pagamentos.exists():
            return JsonResponse({"status": "sem_pagamento"})
        # 🔥 se qualquer um ainda estiver pendente
        if pagamentos.filter(status="pendente").exists():
            return JsonResponse({"status": "pendente"})
        # 🔥 todos pagos
        if pagamentos.filter(status="pago").exists():

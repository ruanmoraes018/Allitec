...
class Mensalidade(models.Model):
    SITUACAO_CHOICES = [('Aberta', 'Aberta'), ('Baixada', 'Baixada')]
    TIPO_PAGAMENTO_CHOICES = [('Boleto', 'Boleto'), ('Pix', 'Pix')]
    situacao = models.CharField(max_length=10, choices=SITUACAO_CHOICES, default='Aberta')
    ...
    tp_mens = models.CharField(max_length=10, choices=TIPO_PAGAMENTO_CHOICES, default='Pix')
    vl_mens = models.DecimalField(max_digits=10, decimal_places=2)
    vl_pago = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    ...
    @property
    def atrasada(self):
        return (self.situacao == 'Aberta' and self.dt_venc is not None and self.dt_venc < date.today())
    @property
    def dias_atraso(self):
        if self.situacao == 'Aberta' and self.dt_venc and self.dt_venc < date.today(): return (date.today() - self.dt_venc).days
        return 0
    @property
    def valor_juros(self):
        if self.dias_atraso <= 0: return Decimal('0.00')
        if not self.vl_juros: return Decimal('0.00')
        if self.tp_juros == 'Percentual':
            return (self.vl_mens * (self.vl_juros / Decimal('100')) * self.dias_atraso)
        else: return self.vl_juros * self.dias_atraso
    @property
    def valor_multa(self):
        if self.dias_atraso <= 0: return Decimal('0.00')
        if not self.vl_multa: return Decimal('0.00')
        if self.tp_multa == 'Percentual': return self.vl_mens * (self.vl_multa / Decimal('100'))
        else: return self.vl_multa
    @property
    ...
    class Meta:
        ...
        ordering = ['-dt_venc']
        indexes = [models.Index(fields=['situacao']), models.Index(fields=['dt_venc']), models.Index(fields=['empresa']),]

...
def lista_mensalidades(request):
    ...
    if not dt_ini and not dt_fim and not por_dt:
        ...
        mensalidades = mensalidades.filter(situacao='Aberta', dt_venc__range=(primeiro_dia, ultimo_dia))
        dt_ini = primeiro_dia.strftime('%d/%m/%Y')
        dt_fim = ultimo_dia.strftime('%d/%m/%Y')
        por_dt = 'Sim'
        sit = 'Aberta'
    if sit in ['Aberta', 'Baixada']: mensalidades = mensalidades.filter(situacao=sit)
    if por_dt == 'Sim' and dt_ini and dt_fim:
        try:
            ...
            if list_p == 'dt_v': mensalidades = mensalidades.filter(dt_venc__range=(dt_ini_dt, dt_fim_dt))
            elif list_p == 'dt_e': mensalidades = mensalidades.filter(created_at__date__range=(dt_ini_dt, dt_fim_dt))
            elif list_p == 'dt_p': mensalidades = mensalidades.filter(dt_pag__range=(dt_ini_dt, dt_fim_dt))
        except ValueError: mensalidades = Mensalidade.objects.none()
    ...
    if emp:
        ...
        if empresa_selecionada:
            ...
            if sit and empresa_selecionada.situacao != sit: empresa_selecionada = None
    if reg == 'todos': num_pagina = mensalidades.count() or 1
    else:
    ...
    return render(request, 'mensalidades/lista.html', {
        'mensalidades': mensalidades, 'empresas': empresas, 'emp': emp, 'sit': sit, 'dt_ini': dt_ini, 'dt_fim': dt_fim, 'p_dt': por_dt, 'list_p': list_p, 'ordem': ordem, 'reg': reg,
    })
...
def att_mensalidade(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            else: return redirect('/mensalidades/lista/?tp=cod&s=' + cid)
        else:
            error_messages = []
            for field in form:
    ...
    else: return render(request, 'mensalidades/att.html', {'form': form, 'mensalidade': mensalidade})
...
def login_portal(request):
    ...
    if request.method == "POST":
        ...
        except Empresa.DoesNotExist: erro = "CNPJ não encontrado"
    return render(request, "mensalidades/login_portal.html", {"erro": erro})

def portal_required(view_func):
    def wrapper(request, *args, **kwargs):
        if not request.session.get("empresa_portal_id"): return redirect('/mensalidades/login/')
        return view_func(request, *args, **kwargs)
    return wrapper

def logout_portal(request):
    if "empresa_portal_id" in request.session: del request.session["empresa_portal_id"]
...
def portal_pagamentos(request):
    ...
    mensalidades = Mensalidade.objects.filter(empresa=empresa, situacao="Aberta", dt_venc__range=(primeiro_dia, ultimo_dia)).order_by('dt_venc')
    return render(request, "mensalidades/portal_pagamentos.html", {"empresa": empresa, "mensalidades": mensalidades,})

def visualizar_pix(request, id):
    cobranca = CobrancaPix.objects.get(mp_payment_id=id)
    return render(request,"mensalidades/pix.html",{"qr_code":cobranca.qr_code_base64,"copia_cola":cobranca.qr_code,"valor":cobranca.valor,"cobranca_id":str(cobranca.mp_payment_id),})

def status_pix(request, id):
    try:
        cobranca = CobrancaPix.objects.get(mp_payment_id=id)
        return JsonResponse({"pago": cobranca.pago})
    except CobrancaPix.DoesNotExist: return JsonResponse({"pago": False})

@portal_required
def gerar_pix_lote_view(request):
    if request.method != "POST": return JsonResponse({"erro": "Método inválido"}, status=405)
    try: data = json.loads(request.body.decode("utf-8"))
    except Exception: return JsonResponse({"erro": "JSON inválido"}, status=400)
    ids = data.get("mensalidades", [])
    if not ids: return JsonResponse({"erro": "Nenhuma mensalidade enviada"}, status=400)
    empresa = Empresa.objects.get(id=request.session["empresa_portal_id"])
    mensalidades = Mensalidade.objects.filter(id__in=ids, empresa=empresa, situacao="Aberta")
    if not mensalidades.exists(): return JsonResponse({"erro": "Mensalidades inválidas"}, status=400)
    cobranca = gerar_pix_lote(mensalidades)   # <-- AQUI
    return JsonResponse({"redirect": f"/mensalidades/pix/{cobranca.mp_payment_id}/"})

@csrf_exempt
def webhook_mp(request):
    if request.GET.get("type") != "payment": return JsonResponse({"ok": True})
    payment_id = request.GET.get("data.id")
    payment_info = sdk.payment().get(payment_id)["response"]
    if payment_info["status"] != "approved": return JsonResponse({"ok": True})
    try: cobranca = CobrancaPix.objects.get(mp_payment_id=payment_id)
    except CobrancaPix.DoesNotExist: return JsonResponse({"erro": "cobranca nao encontrada"})
    cobranca.pago = True
    cobranca.save()

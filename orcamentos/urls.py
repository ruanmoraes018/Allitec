from django.urls import path
from . import views
urlpatterns = [
    path("lista/", views.lista_orcamentos, name='lista-orcamentos'),
    path('detalhes_ajax/<int:codigo>/', views.detalhes_orc_ajax, name='detalhes_orc_ajax'),
    path("add/", views.add_orcamento, name='add-orcamento'),
    path("att/<int:codigo>/", views.att_orcamento, name='att-orcamento'),
    path("clonar/<int:codigo>/", views.clonar_orcamento, name='clonar-orcamento'),
    path("del/<int:codigo>/", views.del_orcamento, name='del-orcamento'),
    path("pdf.orcamento/<int:codigo>/", views.pdf_orcamento_html, name='pdf-orcamento'),
    path("pdf.contrato/<int:codigo>/", views.pdf_contrato_html, name='pdf-contrato'),
    path("pdf.proposta/<int:codigo>/", views.pdf_proposta_html, name='pdf-proposta'),
    path('pdf.producao/<int:codigo>/', views.pdf_producao_html, name='pdf-producao'),
    path('fat.orc/<int:codigo>/', views.faturar_orcamento, name='faturar-orcamento'),
    path('canc.orc/<int:codigo>/', views.cancelar_orcamento, name='cancelar-orcamento'),
    path('comprovante/<int:codigo>/', views.imprimir_comprovante, name='comprovante'),
    path('pdf_a4/<int:codigo>/', views.imprimir_comp_a4, name='pdf-a4'),
    path('enviar-solicitacao/', views.enviar_solicitacao, name='enviar_solicitacao'),
    path('verificar-solicitacao/<int:solicitacao_id>/', views.verificar_status_solicitacao, name='verificar_solicitacao'),
    path('responder-solicitacao/', views.responder_solicitacao, name='responder_solicitacao'),
    path('usuarios-com-permissao/', views.usuarios_com_permissao, name='usuarios_com_permissao'),
    path('liberar-com-senha/', views.liberar_com_senha, name='liberar_com_senha'),
    path('expirar-solicitacao/', views.expirar_solicitacao, name='expirar_solicitacao'),
    path("alterar-status/", views.alterar_status_orcamento, name="alterar_status_orcamento"),
    path("<int:orcamento_id>/gerar-pagamento/", views.gerar_pagamento_orcamento, name='gerar-pagamento-orcamento'),
    path('<int:orcamento_id>/status-pagamento/', views.status_pagamento_orcamento, name='status-pagamento-orcamento'),
    path(
        'recibo/<int:codigo>/',
        views.recibo_orcamento,
        name='recibo_orcamento'
    ),
]

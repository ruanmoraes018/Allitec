...
def regras_js(request):
    ...
    regras = RegraProduto.objects.filter(vinc_emp=empresa, ativo=True).values('codigo', 'tipo', 'expressao_json')
    data = {
        r['codigo']: {'tipo': r['tipo'], 'expressao_json': r['expressao_json'],} for r in regras
    }
...
def aplicar_regras_porta(request):
    try:
        ...
        if isinstance(corpo, list): dados = corpo[0]
        else: dados = corpo
        tabela_id = dados.get('tabela_id')
        contexto = dados.get('contexto', {})
        if not tabela_id: return JsonResponse({'success': False, 'error': 'Tabela não informada'}, status=400)
        ...
        for regra in regras:
            ...
            if regra.tipo == 'QTD':
                if regra.expressao_json:
                    ...
                    if isinstance(itens, str): itens = json.loads(itens)
                    for item in itens:
                        produto = Produto.objects.filter(codigo=item.get('produto_id')).first()
            ...
            if produto_selecionado and qtd_calculada > 0:
                try:
                    ...
                    print(f"Preço não encontrado para produto {produto_selecionado.codigo} na tabela {tabela_id}")
                    continue
...
def att_regra(request, cod_local):
    error_messages = []
    empresa = request.user.empresa
    e = get_object_or_404(RegraProduto, cod_local=cod_local, vinc_emp=empresa)
    ...
    try:
        if request.method == 'POST':
            ...
            else: return redirect('/regras_produto/lista/?tp=cod&s=' + est)
    except ObjectDoesNotExist: error_messages.append("<i class='fa-solid fa-xmark'></i> Objeto não encontrado!")
    except IntegrityError as e: error_messages.append(f"<i class='fa-solid fa-xmark'></i> Erro de integridade: {str(e)}")
    except DatabaseError as e: error_messages.append(f"<i class='fa-solid fa-xmark'></i> Erro de banco de dados: {str(e)}")
    except Exception as e: error_messages.append(f"<i class='fa-solid fa-xmark'></i> Erro inesperado: {str(e)}")

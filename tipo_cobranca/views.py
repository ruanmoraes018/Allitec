...
def lista_tp_cobrancas(request):
    ...
    if tp == 'desc' and s:
        ...
        try: tp_cobrancas = tp_cobrancas.filter(codigo__iexact=s).order_by('descricao')
        except ValueError: tp_cobrancas = TipoCobranca.objects.none()
    if reg == 'todos': num_pagina = tp_cobrancas.count() or 1
    else:
    ...
    return render(request, 'tp_cobrancas/lista.html', {'tp_cobrancas': tp_cobrancas, 's': s, 'tp': tp, 'reg': reg,})
...
def lista_tp_cobrancas_ajax(request):
    ...
    try:
        ...
        results = [{'id': tp_cobranca.codigo, 'text': f"{tp_cobranca.descricao.upper()}"} for tp_cobranca in tp_cobrancas]
...
def att_tp_cobranca(request, codigo):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            else: return redirect('/tp_cobrancas/lista/?tp=cod&s=' + cid)
        else:
            error_messages = []
            for field in form:
                if field.errors: error_messages.append(f"<i class='fa-solid fa-xmark'></i> Campo ({field.label}) é obrigatório!")
            return render(request, 'tp_cobrancas/att.html', {'form': form, 'c': c, 'error_messages': error_messages})
    else: return render(request, 'tp_cobrancas/att.html', {'form': form, 'c': c})

@login_required
def del_tp_cobranca(request, codigo):

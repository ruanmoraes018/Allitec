...
def lista_vendedores(request):
    ...
    if tp == 'desc' and s:
        ...
        try: vendedores = vendedores.filter(codigo__iexact=s).order_by('fantasia')
        except ValueError: vendedores = Vendedor.objects.none()
    if p_dt == 'Sim' and dt_ini and dt_fim:
        ...
        except ValueError: vendedores = vendedores.objects.none()
    if f_s in ['Ativo', 'Inativo']: vendedores = vendedores.filter(situacao=f_s).order_by('fantasia')
    if t_pes in ['Física', 'Jurídica']: vendedores = vendedores.filter(pessoa=t_pes).order_by('fantasia')
    if reg == 'todos': num_pagina = vendedores.count() or 1  # Mostra todas as filials
    else:
    ...
    return render(request, 'vendedores/lista.html', {'vendedores': vendedores, 's': s, 'tp': tp, 't_pes': t_pes, 'dt_ini': dt_ini, 'dt_fim': dt_fim, 'p_dt': p_dt, 'reg': reg})
...
def lista_vendedores_ajax(request):
    ...
    try:
        ...
        results = [{
            'id': vendedor.codigo, 'text': f"{vendedor.fantasia.upper()}", 'situacao': vendedor.situacao,
            'cpfCnpj': vendedor.cpf_cnpj, 'email': vendedor.email, 'tel': vendedor.tel, 'endereco': vendedor.endereco, 'cep': vendedor.cep, 'numero': vendedor.numero,
            'bairro': vendedor.bairro.nome_bairro if vendedor.bairro else '', 'cidade': vendedor.cidade.nome_cidade if vendedor.cidade else '', 'uf': vendedor.uf.nome_estado if vendedor.uf else ''} for vendedor in vendedores]
...
def att_vendedor(request, codigo):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            else: return redirect('/vendedores/lista/?tp=cod&s=' + clie)
        else:
            error_messages = []
            for field in form:
...
@login_required
def del_vendedor(request, codigo):
